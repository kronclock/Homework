...
## 結論
  &#9679; 實作的 BST 能正確處理插入、查找與刪除。  
  &#9679; 隨機均勻分布資料插入使樹高度接近理論 $O(\log n)$，高度比值在合理範圍內。  
  &#9679; 刪除節點函式能正確處理三種情況（無子節點、有一子節點、有兩子節點）。  
  &#9679; 刪除操作的時間複雜度隨樹高呈線性關係，平均時間複雜度約為 $O(\log n)$，最差情況為 $O(n)$。  
  &#9679; 對於大規模資料，建議使用自平衡樹（如 AVL 或紅黑樹）以避免退化。  
